...
- Telemetria rozszerzona o viewer (`viewer_thumbnail_source`, `viewer_signed_url_ms`, `viewer_media_ready_ms`, `viewer_capture_*`) oraz znajomych QR (`friend_qr_scan`, `friend_invite_redeem`) — szczegóły w [`docs/observability.md`](observability.md).
- Logi uploadów w tabeli `upload_logs` + gotowy dashboard SQL — [`docs/upload-observability-dashboard.sql`](upload-observability-dashboard.sql).
- Inbox z embedowanymi listami SwiftUI (`@expo/ui/swift-ui`) — wpływ na scroll/FPS do osobnego profilowania.

**Otwarte ryzyka (bez zmiany produktowej):**

- Background upload — nadal wymaga decyzji produktowej i testów `UIBackgroundModes`.
- Paginacja: RPC `fetch_*_paginated` w SQL jest dostępna; UI w większości nadal opiera się na limitach po stronie klienta — pełny infinite scroll może być kolejną iteracją.
...
- `nixes.thumbnail_b64 TEXT` (idempotent),
- `nixes_thumbnail_b64_size CHECK (octet_length(thumbnail_b64) <= 60000)`,
- `prevent_nix_payload_update` rozszerzony o `thumbnail_b64` jako pole immutable po insercie.
...
- `viewer_thumbnail_source` (`embedded` / `missing`) — udział starych nixów bez `thumbnail_b64` u odbiorcy.
...
- `insertNix` posiada fallback `isMissingThumbnailColumnError` (analogiczny do `client_upload_id`) — apka działa poprawnie na środowiskach bez wykonanej migracji.
- `fetchInboxNixes` i `fetchUnreadInboxQueueFromSender` mają retry-select bez `thumbnail_b64`, gdy kolumna jest nieobecna.
