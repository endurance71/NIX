...
- **Obiektywy (iOS):** nad migawką chipy w stylu Apple Camera — `0,5` / `1×` / `2` (crop wide) / tele (`3`/`5`…) z `NixCameraTorch.getBackLensPresetsAsync`; UI: natywny SwiftUI `Picker` `segmented` (`NativeLensSwitcher`) ze sliding selection + `tint(accent)` jak tab bar. Tylko tylna kamera przy ≥2 opcjach.
