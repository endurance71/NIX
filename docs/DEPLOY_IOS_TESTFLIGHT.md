...
NiX 1.0.6 używa wyłącznie wymaganych trybów tła (`fetch`, `processing`) dla
...
| 2026-07-29 | 1.0.5 | 6 | `221b7e4` | B | production | uploaded ASC | native uploader: unstick PUTs (foreground sessions + thresholds); image/video smoke; buildNumber bumped to 6 locally; dSYM warnings prebuilt frameworks; Internal QA po Processing (do przypisania) |
| 2026-07-29 | 1.0.5 | 7 | working tree (base `221b7e4`) | B | production | uploaded ASC, processing | iOS roadmap release 1–4 behind internal TestFlight flags; Supabase migrations and functions deployed; app + widget build numbers aligned; Internal QA after Processing |
| 2026-07-29 | 1.0.5 | 8 | working tree (base `221b7e4`) | B | production | uploaded to Apple — Internal TestFlight | final roadmap hardening + auth/profile UI; shared invite links explicitly disabled, QR/username retained; uploaded from Xcode Organizer at 14:07; non-blocking missing-dSYM warnings for precompiled Expo/React Native frameworks |
