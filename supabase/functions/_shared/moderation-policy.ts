...
export type ModerationDecision = 'approved' | 'review_required' | 'rejected' | 'error';
...
const HARM_CATEGORIES: readonly HarmCategory[] = ['Hate', 'SelfHarm', 'Sexual', 'Violence'];

export function maxSeverity(analysis: ProviderAnalysis | null | undefined): number | null {
...
export function decideFromProviderAnalysis(
  ...
  if (severity >= 6) {
    return { decision: 'rejected', maxSeverity: severity, policyVersion: POLICY_VERSION };
  }
  if (severity >= 4) {
    const decision: ModerationDecision = options.humanReviewEnabled ? 'review_required' : 'rejected';
    return { decision, maxSeverity: severity, policyVersion: POLICY_VERSION };
  }
  return { decision: 'approved', maxSeverity: severity, policyVersion: POLICY_VERSION };
}

export function decisionFromHttpStatus(status: number): ModerationDecision | null {
