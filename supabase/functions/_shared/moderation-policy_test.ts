...
import { decideFromProviderAnalysis, decisionFromHttpStatus, POLICY_VERSION } from './moderation-policy.ts';
...
Deno.test('severity 4 is rejected when human review is off', () => {
  const result = decideFromProviderAnalysis(syntheticReview, { humanReviewEnabled: false });
  assertEquals(result.decision, 'rejected');
});

Deno.test('severity 4 is review_required when human review is on', () => {
  const result = decideFromProviderAnalysis(syntheticReview, { humanReviewEnabled: true });
